...
기본 사용:
    ...
    from src.infer_diff import get_diff
    ...
    # -> {"new": [str, ...],
    #     "conflict": [{known_text, new_text, reason, doc_id?, severity?}, ...]}
...
# src/ 디렉토리 외부에서 import 될 때도 prompt_text 를 찾도록 sys.path 보강.
...
class Conflict(BaseModel):
    ...
    # 현재 학습된 모델(schema v2)은 doc_id/severity 를 output 에 포함하지 않는다.
    # guided_json 이 required 로 강제하면 hallucination 이 발생하므로 Optional 로 둔다.
    # 후속 학습(iteration)에서 출력하도록 바뀌면 그대로 파싱된다.
    doc_id: Optional[str] = Field(
        default=None,
        description="ID of the KNOWN_DOC where `known_text` appears (optional).",
    )
    severity: Optional[Literal["low", "medium", "high"]] = Field(
        default=None,
        description="Conflict severity: low/medium/high (optional).",
    )
...
def get_diff(
    ...
    *,
    ...
    Returns:
        ...
        `known_text / new_text / reason` 필드 + optional `doc_id / severity`).
        guided_json 으로 스키마를 강제하므로 항상 위 형태를 만족한다.
    """
...
if __name__ == "__main__":
    # Smoke test: `python src/infer_diff.py` 로 간단한 샘플 실행.
