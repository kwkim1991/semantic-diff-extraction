...
#   1. HF -> Megatron checkpoint conversion (src/convert_hf_ckpt.py)
#   2. Dataset conversion (python src/convert_data.py)
#
# Usage:
#   bash scripts/run_sft.sh <MEGATRON_CKPT_DIR> [NGPU]
#
# GPU 0 가 vLLM 등에 물려있어 1,2,3 번만 쓰고 싶을 때:
#   CUDA_VISIBLE_DEVICES=1,2,3 bash scripts/run_sft.sh <ckpt>
...
CMD=(torchrun --nproc_per_node="${NGPU}" src/sft_nemotron_lora.py
